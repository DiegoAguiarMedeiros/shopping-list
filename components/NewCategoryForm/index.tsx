...
export type NewCategoryFormProps = {
  ...
  action: "addCategory" | "editCategory" | "copyCategory";
  items?: List;
};
...
const NewCategoryForm = ({
  ...
  const handleCopyCategory = (): void => {
    if (newItem.item) {
      closeBottomSheet();
      const newList = returnNewList();
      const selectedItem = list.find((item) => item.uuid === items?.uuid!);
      if (selectedItem) {
        const returnHandleCopyListItem = handleCopyCategoryItem(selectedItem.items);
